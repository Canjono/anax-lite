...
$app->router->add("admin", function () use ($app) {
    if (!$app->user->isAdmin()) {
        ...
        $app->view->add("admin/admin", ["region" => "main","users" => $usersTable, "max" => $max], "main", 0);
...
$app->router->add("admin/doUpdate", function () use ($app) {
    if (!$app->user->isAdmin()) {
        // Redirect to home page if already logged in
        header("Location: {$app->url->create('')}");
    } else {
        ...
        $updateRoute = $app->url->create("admin/update?user={$oldUsername}");
...
$app->router->add("admin/add/validate", function () use ($app) {
    if (!$app->user->isAdmin()) {
        // Redirect to home page if already logged in
        header("Location: {$app->url->create('')}");
    } else {
        ...
        if ($user != null && $pass != null && $rePass != null && $permission != null) {
            if ($app->query->exists($user)) {
                ...
                header("Location: {$app->url->create('admin/add')}");
            }
        } else {
